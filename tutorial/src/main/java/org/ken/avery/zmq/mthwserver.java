...
public class mthwserver
{
    ...
    private static final class WorkerThread extends Thread
    {
        ...
        public void run()
        {
            final ZMQ.Socket receiver = context.socket(ZMQ.REP);
            receiver.connect("inproc://workers");
            while (!Thread.currentThread().isInterrupted())
            {
                final byte[] request = receiver.recv(0);
                ...
                final String replyString = "World" + " ";
                final byte[] reply = replyString.getBytes();
    ...
    public static void main(final String[] args)
    {
        ...
        final ZMQQueue queue = new ZMQQueue(context, clients, workers);
